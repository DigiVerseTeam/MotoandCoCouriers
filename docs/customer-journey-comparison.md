...
This comparison checks the uploaded user journeys against the active Moto & Co portal. It does not invent missing workflow detail. Anything not implemented or not confirmed remains visible as a gap.

2026-07-02 reconciliation: offline driver behaviour is no longer an undefined gap. It is now governed by `SOP-OPS-01 Offline Device Sync And Recovery`; field UAT on the real driver device/browser remains required.
...
- V1 invoice PDF download is built for Admin manual email outside the portal. No Xero/OpenClaw/accounting API integration is in the V1 baseline.
...
- Driver-created daily run workflow after ERD review; previous night-before lockdown is rejected.
- Production reason-code governance for Failed Delivery, driver issues, retained goods, upload retry, and wrong-address handling. Offline handling is governed by SOP-OPS-01.
...
- Driver offline recovery field UAT against SOP-OPS-01.
...
- BAS/tax accountant handoff and Otimi Rules reporting cadence/format/recipient remain out-of-system human/accounting processes unless separately approved.
...
- V1 invoice PDF download is built. Exact external email template, actual email dispatch, bounce handling, and export remain open.
...
- Production retention destruction/review authority remains open until role-based GM Moto & Co Logistics Privacy Owner evidence, contact details, and approval workflow are retained.
...
- Driver offline/upload retry is governed by SOP-OPS-01, but field UAT and the unrecoverable local outbox recovery procedure remain open.
...
- Failed Delivery has Policy #8 attempt/evidence, Admin fee-review, waiver, and approved-fee billing behavior, but final production reason codes remain open. Offline retry handling is governed by SOP-OPS-01.
