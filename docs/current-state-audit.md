...
| Supabase backend | Active logistics migrations are applied to production with RLS, API table privileges, private POD storage, audit, retention, pricing, notification, cancellation, billing-notice generation, Policy #6 NDB suspected-incident controls, Policy #21 / Policy #7 data-use controls, Policy #3 / Policy #4 / Policy #5 privacy request controls, Policy #24 financial reconciliation controls, Policy #20 AI draft review controls, Policy #22 driver scheduling, Policy #27 WHS hazard controls, CAP-MCL-001 supplier approval gate, CAP-MCL-002 run planning monitor, Policy #8, Policy #15, Policy #16, Policy #18 dispute SLA/remedy/outcome-history, SOP-PUP-02 supplier-stop closeout, SOP-DEL-01 grouped delivery stops, SOP-DEL-04 delivery sign-off proof, SOP-DEL-05, UJ-DRV-001 run-close confirmation, a bootstrapped Admin role, the SOP-IAM-03 Super Admin provisioning migration/script, and `gerrard@otimi.com.au` bootstrapped as first Super Admin with audit/runtime evidence. HCM-owned driver legal/classification/agreement/conduct/expansion migrations were extracted from the active migration set. | Production backend active; approved launch master data and live actor journey tests still blocked |
...
- `gerrard@otimi.com.au` was upgraded from Admin to first Super Admin in production on 2026-06-20 using the user approval reference `User approved gerrard@otimi.com.au Super Admin upgrade in Codex chat - 2026-06-20`. Direct Supabase verification confirmed profile role `super_admin`, active `super_admin` assignment `ACT-INT-003`, one `master_data_changes` audit row, and one `runtime_records` bootstrap evidence row.
- Production provisioning endpoint smoke on `https://motoandcocouriers.vercel.app/api/admin/provision-user` returned `401 Missing Supabase bearer token` for unauthenticated GET and POST on 2026-06-20.
- `npm.cmd run verify:live` passed 32 production checks on 2026-06-20, then failed only on missing approved client/supplier/driver/vehicle records and missing active `driver`, `client_ops`, and `client_billing` role records.
...
- Live Super Admin login/browser testing still needs a real Supabase session for `gerrard@otimi.com.au`; the database bootstrap is complete, but the UI journey needs the email login link/session.
- Vercel is deployed, but full production actor testing remains blocked until approved launch master data and active Driver/Client Ops/Client Billing role records exist.
