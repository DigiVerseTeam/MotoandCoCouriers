...
| Production Supabase workflow wiring is not complete. | Supabase project `fhrqfrhqopicekaiibyj` is active in `ap-southeast-2`; active logistics migrations are applied; private `delivery-proof` bucket exists; 8 source-backed price rules and matching change-log rows are seeded; Vercel production env has Supabase URL, publishable key, server secret, project ref, and region. | Replace remaining local/mock actor workflow persistence with Supabase reads/writes and repeat actor workflow verification against production. |
...
| Live RLS/Auth binding is untested with real users. | First-pass BOAS Sheet 05 RLS migration is applied and all 40 public base tables have RLS enabled; local login still uses generated testing codes. | Configure Supabase Auth identity claims/role mapping, then run live RLS tests for Client Operational, Client Billing, Driver, Admin, and Receiver no-login boundaries. |
| Private POD Storage upload flow is not wired into the app. | Private `delivery-proof` bucket exists with `public=false`; migrations define `deliveries/{delivery_id}/...` path contract, assigned-driver/Admin upload policy, linked-role read policy, SOP-DEL-04 proof sign-off fields/constraints, and retention queueing. | Confirm upload transport choice, device model/offline behavior, signed-upload/RPC/browser-upload authority, and wire production signature upload. |
| Vercel production deployment is live but actor workflows still use local/mock state. | Vercel project `digi-verse/motoandcocouriers` is connected to GitHub and deployed at `https://motoandcocouriers.vercel.app`; `/`, `/login`, `/booking`, and `/admin` returned HTTP 200 on 2026-06-19; Supabase production env values are set. | Redeploy after Supabase workflow wiring and repeat live actor workflow verification. Preview deployments must not point at production Supabase. |
