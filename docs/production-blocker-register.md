...
| Production invoice dispatch is unconfirmed. | Admin can create draft invoices, preview line items, and confirm the rendered invoice is correct under SOP-BIL-04; that single action records local dispatch evidence. | Confirm Zoho Books integration versus export/manual reconciliation, invoice PDF/email template, dispatch provider, bounce handling, and accounting handoff. |
...
| Production reason-code taxonomy is incomplete. | Local No Pickup now uses the decision-register confirmed category list, including `time_constraint`; SOP-RUN-04 Bring Forward is modelled as early collection of a future pickup from a supplier already on today's route; Failed Delivery now uses source-backed SOP-DEL-04 categories. | Confirm any final production reason codes beyond the SOP-DEL-04 categories for driver issues, redelivery, retained goods, offline/upload retry, and any extra wrong-address handling. |
