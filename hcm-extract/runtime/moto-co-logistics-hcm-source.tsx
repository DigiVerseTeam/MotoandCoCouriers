...
const POLICY13_DRIVER_VERIFICATION_SOURCE = "Policy #13 / POL-OPS-013";
const POLICY19_DRIVER_CONDUCT_SOURCE = "Policy #19 / POL-OPS-019";
...
const POLICY13_LICENCE_OUTCOME_VERIFIED = "Current valid Australian licence sighted and copy retained";
const POLICY13_PHOTO_ID_OUTCOME_VERIFIED = "Government photo ID sighted";
const POLICY13_CRIMINAL_HISTORY_OUTCOME_CLEAR = "No disqualifying criminal history recorded";
const POLICY13_RIGHT_TO_WORK_OUTCOME_VERIFIED = "Right to work in Australia confirmed";
const POLICY25_MULTI_DRIVER_SOURCE = "Policy #25 / POL-OPS-025";
const POLICY26_COURIER_BILLING_SOURCE = "Policy #26 / POL-OPS-026";
...
const POLICY19_CONDUCT_ISSUE_TYPES = ["Conduct / Complaint Concern", "Supplier or Client Staff Misconduct"];
...
const seedDrivers = [
  {
    ...
    licenceNumber: "",
    licenceClass: "",
    licenceOriginalSightedDate: "",
    licenceCopyRetained: false,
    licenceVerifiedDate: "",
    licenceVerificationOutcome: "",
    photoIdVerifiedDate: "",
    photoIdVerificationOutcome: "",
    criminalHistoryReviewedDate: "",
    criminalHistoryOutcome: "",
    rightToWorkVerifiedDate: "",
    rightToWorkOutcome: "",
    licenceCurrencyReviewedDate: "",
    knownLicenceIssue: false,
    licenceIssueNote: "",
    policy13AdminIdentifier: "",
    classification: "",
    classificationStatus: "Blocked pending Fair Work-aware legal classification",
    engagementStartDate: "",
    engagementEndDate: "",
    onboardingStatus: "Production onboarding blocked",
    lastReviewed: "",
    notes: "SOP-JDD-01 requires the current driver's engagement start date to be recorded in APP-ADM-006 now.",
  },
];
...
const DRIVER_CLASSIFICATION_BLOCKER = "Blocked pending Fair Work-aware legal classification";
const DRIVER_ONBOARDING_BLOCKER = "Production onboarding blocked";
function normaliseDriverRecord(driver = {}) {
  const classification = driver.classification || "";
  return {
    ...driver,
    name: driver.name || "",
    email: driver.email || "",
    phone: driver.phone || "",
    licenceNumber: driver.licenceNumber || driver.licence_number || "",
    licenceClass: driver.licenceClass || driver.licence_class || "",
    licenceOriginalSightedDate: optionalIsoDate(driver.licenceOriginalSightedDate || driver.licence_original_sighted_date),
    licenceCopyRetained: Boolean(driver.licenceCopyRetained ?? driver.licence_copy_retained),
    licenceVerifiedDate: optionalIsoDate(driver.licenceVerifiedDate || driver.licence_verified_date),
    licenceVerificationOutcome: driver.licenceVerificationOutcome || driver.licence_verification_outcome || "",
    photoIdVerifiedDate: optionalIsoDate(driver.photoIdVerifiedDate || driver.photo_id_verified_date),
    photoIdVerificationOutcome: driver.photoIdVerificationOutcome || driver.photo_id_verification_outcome || "",
    criminalHistoryReviewedDate: optionalIsoDate(driver.criminalHistoryReviewedDate || driver.criminal_history_reviewed_date),
    criminalHistoryOutcome: driver.criminalHistoryOutcome || driver.criminal_history_outcome || "",
    rightToWorkVerifiedDate: optionalIsoDate(driver.rightToWorkVerifiedDate || driver.right_to_work_verified_date),
    rightToWorkOutcome: driver.rightToWorkOutcome || driver.right_to_work_outcome || "",
    licenceCurrencyReviewedDate: optionalIsoDate(driver.licenceCurrencyReviewedDate || driver.licence_currency_reviewed_date),
    knownLicenceIssue: Boolean(driver.knownLicenceIssue ?? driver.known_licence_issue),
    licenceIssueNote: driver.licenceIssueNote || driver.licence_issue_note || "",
    policy13AdminIdentifier: driver.policy13AdminIdentifier || driver.policy_13_admin_identifier || "",
    policy13VerificationSource: driver.policy13VerificationSource || driver.policy_13_verification_source || POLICY13_DRIVER_VERIFICATION_SOURCE,
    classification,
    classificationStatus: driver.classificationStatus || driver.classification_status || (classification ? "Recorded from legal advice" : DRIVER_CLASSIFICATION_BLOCKER),
    engagementStartDate: optionalIsoDate(driver.engagementStartDate || driver.engagement_start_date),
    engagementEndDate: optionalIsoDate(driver.engagementEndDate || driver.engagement_end_date),
    onboardingStatus: driver.onboardingStatus || driver.onboarding_status || DRIVER_ONBOARDING_BLOCKER,
    lastReviewed: optionalIsoDate(driver.lastReviewed || driver.last_reviewed),
    notes: driver.notes || "",
  };
}
function driverPolicy13Gaps(driver = {}, runDate = todayBrisbane()) {
  const record = normaliseDriverRecord(driver);
  const gaps = [];
  if (!record.licenceNumber) gaps.push("licence number not recorded");
  if (!record.licenceClass) gaps.push("licence class for van not recorded");
  if (!record.licenceOriginalSightedDate) gaps.push("original licence sighting date required");
  if (!record.licenceCopyRetained) gaps.push("licence copy retention not confirmed");
  if (!record.licenceVerifiedDate) gaps.push("licence verification date required");
  if (record.licenceVerificationOutcome !== POLICY13_LICENCE_OUTCOME_VERIFIED) gaps.push("licence outcome must confirm current valid Australian licence");
  if (!record.photoIdVerifiedDate) gaps.push("photo ID sighting date required");
  if (record.photoIdVerificationOutcome !== POLICY13_PHOTO_ID_OUTCOME_VERIFIED) gaps.push("government photo ID outcome not verified");
  if (!record.criminalHistoryReviewedDate) gaps.push("criminal-history relevance review date required");
  if (record.criminalHistoryOutcome !== POLICY13_CRIMINAL_HISTORY_OUTCOME_CLEAR) gaps.push("criminal-history outcome not cleared");
  if (!record.rightToWorkVerifiedDate) gaps.push("right-to-work verification date required");
  if (record.rightToWorkOutcome !== POLICY13_RIGHT_TO_WORK_OUTCOME_VERIFIED) gaps.push("right-to-work outcome not confirmed");
  if (!record.policy13AdminIdentifier) gaps.push("Admin identifier for verification checks required");
  if (record.knownLicenceIssue) gaps.push("known licence issue requires removal from run schedule");
  const annualReviewDate = record.licenceCurrencyReviewedDate || record.licenceVerifiedDate;
  if (!annualReviewDate) gaps.push("annual licence currency review date required");
  if (annualReviewDate && addYears(annualReviewDate, 1) <= runDate) gaps.push(`annual licence currency review due by ${fmtFullDate(addYears(annualReviewDate, 1))}`);
  return gaps;
}
function driverPolicy13Ready(driver = {}, runDate = todayBrisbane()) {
  return driverPolicy13Gaps(driver, runDate).length === 0;
}
function driverPolicy13Status(driver = {}, runDate = todayBrisbane()) {
  const gaps = driverPolicy13Gaps(driver, runDate);
  if (gaps.length === 0) return { status: "Verified", badgeClass: "b-done", gaps };
  if (normaliseDriverRecord(driver).knownLicenceIssue) return { status: "Licence Issue", badgeClass: "b-cancelled", gaps };
  return { status: "Verification Blocked", badgeClass: "b-pending", gaps };
}
function driverAssignmentFixtureNotice(driver = {}, runDate = todayBrisbane()) {
  const gaps = driverPolicy13Gaps(driver, runDate);
  if (gaps.length === 0) return null;
  const preview = gaps.slice(0, 3).join("; ");
  const extra = gaps.length > 3 ? ` and ${gaps.length - 3} more Policy #13 item${gaps.length - 3 === 1 ? "" : "s"}` : "";
  return {
    gaps,
    text: `Local verification fixture only: production APP-ADM-002 assignment remains blocked until ${preview}${extra}.`,
  };
}
function driverRecordGaps(driver = {}) {
  const record = normaliseDriverRecord(driver);
  const gaps = [];
  if (!record.engagementStartDate) gaps.push("APP-ADM-006 engagement start date required now");
  if (!record.lastReviewed) gaps.push("Admin review date not recorded");
  gaps.push(...driverPolicy13Gaps(record).map(gap => `Policy #13: ${gap}`));
  return gaps;
}
function driverOnboardingBlocked(driver = {}) {
  const record = normaliseDriverRecord(driver);
  return record.classificationStatus === DRIVER_CLASSIFICATION_BLOCKER || record.onboardingStatus === DRIVER_ONBOARDING_BLOCKER;
}
function driverRecordBadgeClass(driver = {}) {
  if (!normaliseDriverRecord(driver).engagementStartDate) return "b-cancelled";
  return driverOnboardingBlocked(driver) ? "b-pending" : "b-done";
}
function driverRetentionUntil(driver = {}) {
  const endDate = optionalIsoDate(driver.engagementEndDate || driver.engagement_end_date);
  return endDate ? addYears(endDate, 7) : "";
}
function activeDriverRecords(drivers = []) {
  return (drivers || [])
    .map(normaliseDriverRecord)
    .filter(driver => !["inactive", "archived", "terminated"].includes(String(driver.onboardingStatus || "").toLowerCase()));
}
function policy25MultiDriverReadiness(drivers = []) {
  const activeDrivers = activeDriverRecords(drivers);
  const triggerReached = activeDrivers.length >= 3;
  const additionalDriverPreRunReviewDue = activeDrivers.length >= 2;
  const driverVerificationGaps = activeDrivers.flatMap(driver => {
    const gaps = driverRecordGaps(driver);
    if (driverOnboardingBlocked(driver)) gaps.push("legal classification / onboarding blocker remains");
    return gaps.length ? [`${driver.name || driver.id}: ${gaps.join(", ")}`] : [];
  });
  const blockers = [
    "Policy #25 is Conditional - Not Active until the driver pool reaches 3+ active drivers.",
    "APP-ADM-002 multi-driver assignment logic review is not recorded in the local build.",
    "Admin policy register activation is not recorded in the local build.",
  ];
  if (driverVerificationGaps.length) blockers.push(...driverVerificationGaps);
  return {
    activeDriverCount: activeDrivers.length,
    triggerReached,
    additionalDriverPreRunReviewDue,
    status: triggerReached ? "Activation Review Required" : "Conditional - Not Active",
    badgeClass: triggerReached || additionalDriverPreRunReviewDue ? "b-pending" : "b-done",
    blockers,
  };
}
function policy26CourierBillingReadiness() {
  return {
    status: "Placeholder - Not Active",
    badgeClass: "b-pending",
    blockers: [
      "Business decision to engage external couriers or introduce a per-run model is not confirmed.",
      "External courier employment/contractor classification legal advice is not confirmed.",
      "Per-run rate, payment cycle, expenses, GST treatment, and contractor invoicing requirements are not confirmed.",
      "External courier APP-DRV-003 or equivalent POD integration is not confirmed.",
      "Owner/Admin joint activation is not recorded.",
    ],
  };
}
...
function DriverPortal({ user, orders, priceRules, exceptions, runClosures, onUpdateOrder, onUpdateOrders, onDeliveryProof, onException, onRunClose, onLogout }) {
  ...
  const assignmentFixture = activeRunDates
    .map(runDate => driverAssignmentFixtureNotice(user, runDate))
    .find(Boolean);
  ...
  function reportRunIssue() {
    ...
    const policy19Issue = POLICY19_CONDUCT_ISSUE_TYPES.includes(runIssueDraft.type);
    ...
    onException({
      ...
      owner: "Admin",
      note: policy19Issue
        ? `${POLICY19_DRIVER_CONDUCT_SOURCE}: ${user.name}: ${note}. Driver must report to Admin and not attempt direct resolution. Disciplinary process remains subject to legal classification and disciplinary-process review.`
        : ["Pre-Trip Defect", "Fatigue / Health Concern", "WHS Incident / Near Miss"].includes(runIssueDraft.type)
        ? `${POLICY27_WHS_SOURCE}: ${user.name}: ${note}`
        : `${user.name}: ${note}`,
      status: "Open",
      driverId: user.id,
      severity: runIssueDraft.type === "Missing Stop" ? "Medium" : "High",
      source: policy19Issue
        ? `${POLICY19_DRIVER_CONDUCT_SOURCE} / APP-ADM-005`
        : policy27Issue
        ? `${POLICY27_WHS_SOURCE} / APP-ADM-002`
        : "APP-DRV-001 run issue",
      conductPolicyRef: policy19Issue ? POLICY19_DRIVER_CONDUCT_SOURCE : "",
      conductCategory: policy19Issue ? runIssueDraft.type : "",
    });
          ...
          <>
              ...
              <div className="fr">
                <div className="f"><label>Issue Type</label>
                  <select value={runIssueDraft.type} onChange={e => setRunIssueDraft(p => ({ ...p, type: e.target.value }))}>
                    ...
                    <option value="Conduct / Complaint Concern">Conduct / Complaint Concern</option>
                    <option value="Supplier or Client Staff Misconduct">Supplier or Client Staff Misconduct</option>
            ...
            {active.length > 0 && (
              <div className="card">
                ...
                {assignmentFixture && (
                  <div style={{ fontSize: ".82rem", color: T.acc, marginTop: ".55rem" }}>
                    {assignmentFixture.text}
                  </div>
                )}
                <div style={{ fontSize: ".82rem", color: T.mu, marginTop: ".45rem" }}>
...
function AdminPortal({ orders, clients, drivers, vehicles = [], suppliers, priceRules, exceptions, audit, masterDataChanges, invoices, billingNotices, operationalNotices = [], proofs, exceptionAlerts, driverAvailability, financialReconciliations = [], aiDrafts = [], dataBreachIncidents = [], dataUseRecords = [], privacyRequests = [], accessRecords, runClosures = [], onUpdateOrder, onUpdateOrders, onUpdateClient, onSaveSupplier, onArchiveSupplier, onSavePriceRule, onSaveVehicle, onSaveDriver, onCreateInvoice, onUpdateInvoice, onRecordBillingNotice, onSaveFinancialReconciliation, onCreateAiDraft, onUpdateAiDraft, onSaveDataBreachIncident, onSaveDataUseRecord, onSavePrivacyRequest, onSaveAccessChange, onCreateSupplierReviewException, onCreateSupplierPickupStandardsException, onCreatePricingReviewException, onCreateUnmatchedBillingException, onCreateRunPlanningException, onAcknowledgeException, onUpdateException, onAcknowledgeExceptionAlert, onSaveDriverAvailability, onLogout }) {
  ...
  const compileDriverPolicy13 = compileDriver ? driverPolicy13Status(compileDriver, compileRunDate) : { status: "Driver Required", badgeClass: "b-cancelled", gaps: ["Driver required"] };
  ...
  const driverRecordRows = (drivers || []).map(driver => {
    const record = normaliseDriverRecord(driver);
    const policy13 = driverPolicy13Status(record);
    return {
      driver: record,
      gaps: driverRecordGaps(record),
      policy13,
      onboardingBlocked: driverOnboardingBlocked(record),
      retentionUntil: driverRetentionUntil(record),
    };
  });
  const driverEngagementStartMissing = driverRecordRows.filter(row => !row.driver.engagementStartDate).length;
  const policy13BlockedDriverCount = driverRecordRows.filter(row => row.policy13.gaps.length > 0).length;
  const policy25Readiness = policy25MultiDriverReadiness(drivers);
  const policy26Readiness = policy26CourierBillingReadiness();
  ...
  function saveDriverRecordDraft() {
    ...
    if (!driverDraft.engagementStartDate) return showWorkflowNotice("APP-ADM-006 requires the current driver's engagement start date to be recorded now.");
    ...
    onSaveDriver(normaliseDriverRecord({
      ...
      licenceNumber: driverDraft.licenceNumber.trim(),
      licenceClass: driverDraft.licenceClass.trim(),
      licenceOriginalSightedDate: driverDraft.licenceOriginalSightedDate,
      licenceCopyRetained: Boolean(driverDraft.licenceCopyRetained),
      licenceVerifiedDate: driverDraft.licenceVerifiedDate,
      licenceVerificationOutcome: driverDraft.licenceVerificationOutcome,
      photoIdVerifiedDate: driverDraft.photoIdVerifiedDate,
      photoIdVerificationOutcome: driverDraft.photoIdVerificationOutcome,
      criminalHistoryReviewedDate: driverDraft.criminalHistoryReviewedDate,
      criminalHistoryOutcome: driverDraft.criminalHistoryOutcome,
      rightToWorkVerifiedDate: driverDraft.rightToWorkVerifiedDate,
      rightToWorkOutcome: driverDraft.rightToWorkOutcome,
      licenceCurrencyReviewedDate: driverDraft.licenceCurrencyReviewedDate,
      knownLicenceIssue: Boolean(driverDraft.knownLicenceIssue),
      licenceIssueNote: driverDraft.licenceIssueNote.trim(),
      policy13AdminIdentifier: driverDraft.policy13AdminIdentifier.trim(),
      policy13VerificationSource: POLICY13_DRIVER_VERIFICATION_SOURCE,
      classification: driverDraft.classification.trim(),
      classificationStatus: driverDraft.classificationStatus || (driverDraft.classification.trim() ? "Recorded from legal advice" : DRIVER_CLASSIFICATION_BLOCKER),
      onboardingStatus: driverDraft.onboardingStatus || DRIVER_ONBOARDING_BLOCKER,
  ...
  function exceptionInvestigationControl(exception) {
    ...
    if (POLICY19_CONDUCT_ISSUE_TYPES.includes(exception.type)) {
      return {
        policy: `${POLICY19_DRIVER_CONDUCT_SOURCE} / APP-ADM-005`,
        evidence: "Driver report, supplier/client interaction context, complaint or concern evidence, and relationship risk review",
        defaultOutcome: `${exception.type} reviewed`,
        notePlaceholder: "Record parties involved, what Admin investigated, whether direct driver resolution was avoided, relationship follow-up, and any remaining legal classification or disciplinary-process blocker before consequences.",
      };
    }
  ...
  function compileRun() {
    ...
    const policy13Gaps = driverPolicy13Gaps(driver, compileRunDate);
    if (policy13Gaps.length) return showWorkflowNotice(`Policy #13 blocks ${driver.name} assignment: ${policy13Gaps.join("; ")}.`);
  ...
  function assignDispatch(order) {
    ...
    if (!vehicleState.ready) return showWorkflowNotice(`Vehicle cannot be assigned: ${vehicleState.reason}.`);
    const policy13Gaps = driverPolicy13Gaps(driver, runDate);
    if (policy13Gaps.length) return showWorkflowNotice(`Policy #13 blocks ${driver.name} assignment: ${policy13Gaps.join("; ")}.`);
      ...
      <div className="main">
        ...
        {view === "dispatch" && (
          <>
            ...
            <div className="card">
              ...
              {compileDriver && (
                <div className="pills" style={{ marginTop: ".5rem" }}>
                  <span className={`pill${driverPolicy13Ready(compileDriver, compileRunDate) ? " sel" : ""}`}>{POLICY13_DRIVER_VERIFICATION_SOURCE}: {compileDriverPolicy13.status}</span>
                  <span className="pill">Licence {compileDriver.licenceNumber || "not recorded"}</span>
                  <span className="pill">Annual review {(compileDriver.licenceCurrencyReviewedDate || compileDriver.licenceVerifiedDate) ? fmt(compileDriver.licenceCurrencyReviewedDate || compileDriver.licenceVerifiedDate) : "not recorded"}</span>
                </div>
              )}
              ...
              {compileDriverPolicy13.gaps.length > 0 && (
                <div className="err" style={{ marginTop: ".8rem" }}>
                  Policy #13 blocks driver assignment: {compileDriverPolicy13.gaps.join("; ")}
                </div>
              )}
              ...
              <button className="btn b-acc" style={{ marginTop: ".9rem" }} disabled={!compileCandidates.length || Boolean(compileAvailabilityBlock) || !selectedVehicleCompliance.ready || compileDriverPolicy13.gaps.length > 0} onClick={compileRun}>
            ...
            {unassignedDispatch.map(o => {
              ...
              const selectedDriver = drivers.find(driver => driver.id === (dispatchDraft.driverId || drivers[0]?.id));
              const driverVerificationGaps = selectedDriver ? driverPolicy13Gaps(selectedDriver, o.actualRunDate || o.date) : ["Driver required"];
              const dispatchBlocked = Boolean(availabilityBlock) || !vehicleState.ready || driverVerificationGaps.length > 0;
              return (
                <div className="card" key={o.id}>
                  <div className="card-head"><div className="card-title">{o.id} - {o.clientName}</div>{statusBadge(o.status)}</div>
                  ...
                  {driverVerificationGaps.length > 0 && <div style={{ fontSize: ".8rem", color: T.acc, margin: ".45rem 0" }}>Policy #13 blocks {selectedDriver?.name || "selected driver"}: {driverVerificationGaps.join("; ")}</div>}
                  <button className="btn b-teal b-sm" disabled={dispatchBlocked} onClick={() => assignDispatch(o)}>{driverVerificationGaps.length ? "Driver Verification Blocked" : availabilityBlock ? "Driver Unavailable" : !vehicleState.ready ? "Vehicle Blocked" : "Assign Single Stop"}</button>
                </div>
              );
            })}
            ...
            {assignedDispatch.map(o => {
              ...
              const assignmentFixture = driver ? driverAssignmentFixtureNotice(driver, o.actualRunDate || o.date) : null;
              return (
                <div className="card" key={o.id}>
                  <div className="card-head"><div className="card-title">{o.id} - {o.clientName}</div>{statusBadge(o.status)}</div>
                  <div className="meta">
                  ...
                  {assignmentFixture && <div style={{ fontSize: ".8rem", color: T.acc, margin: ".45rem 0" }}>{assignmentFixture.text}</div>}
                  <div style={{ display: "flex", gap: ".6rem", flexWrap: "wrap" }}>
        ...
        {view === "privacy" && (
          <>
            ...
            <div className="card">
              ...
              <div className="f"><label>Personal Information Categories *</label><textarea value={privacyRequestDraft.piiCategories} onChange={e => setPrivacyRequestDraft(prev => ({ ...prev, piiCategories: e.target.value }))} placeholder="Account contact, delivery address, order history, driver licence, receiver signature..." /></div>
              <div className="grid2">
        ...
        {view === "drivers" && (
          <>
            ...
            <div className="stats">
              <div className="stat"><div className="stat-num" style={{ color: T.tx }}>{drivers.length}</div><div className="stat-lbl">Driver Records</div></div>
              <div className="stat"><div className="stat-num" style={{ color: driverEngagementStartMissing ? T.red : T.tx }}>{driverEngagementStartMissing}</div><div className="stat-lbl">Engagement Start Missing</div></div>
              <div className="stat"><div className="stat-num" style={{ color: policy13BlockedDriverCount ? T.red : T.tx }}>{policy13BlockedDriverCount}</div><div className="stat-lbl">Policy #13 Blocked</div></div>
              <div className="stat"><div className="stat-num" style={{ color: T.acc }}>{driverRecordRows.filter(row => row.onboardingBlocked).length}</div><div className="stat-lbl">Onboarding Blocked</div></div>
            </div>
            <div className="card">
              <div className="card-head">
                <div className="card-title">APP-ADM-006 Driver Record Governance</div>
                <span className="badge b-pending">SOP-JDD-01</span>
              </div>
              <div style={{ fontSize: ".82rem", color: T.mu }}>
                Record the current driver's engagement start date now. APP-JDD-003 production onboarding remains blocked until a Fair Work-aware lawyer confirms driver classification and production onboarding authority.
              </div>
            </div>
            <div className="card">
              <div className="card-head">
                <div className="card-title">Policy #13 Driver Verification Gate</div>
                <span className={`badge ${policy13BlockedDriverCount ? "b-cancelled" : "b-done"}`}>{policy13BlockedDriverCount ? `${policy13BlockedDriverCount} Blocked` : "Clear"}</span>
              </div>
              <div className="meta">
                <span>{POLICY13_DRIVER_VERIFICATION_SOURCE}</span>
                <span>No provisional assignment</span>
                <span>Annual licence currency review</span>
                <span>Criminal-history scope remains an open legal item</span>
              </div>
              <div style={{ fontSize: ".82rem", color: T.mu }}>
                Admin must document licence, original sighting, retained copy, government photo ID, criminal-history relevance, right-to-work, outcome date, and Admin identifier before APP-ADM-002 can assign the driver to a run.
              </div>
            </div>
            <div className="card">
              <div className="card-head">
                <div className="card-title">Conditional Driver / Courier Expansion Gates</div>
                <span className={`badge ${policy25Readiness.badgeClass}`}>{policy25Readiness.activeDriverCount} Active Driver{policy25Readiness.activeDriverCount === 1 ? "" : "s"}</span>
              </div>
              <div className="grid2">
                <div style={{ border: `1px solid ${T.border}`, borderRadius: 8, padding: ".75rem" }}>
                  <div className="card-head" style={{ marginBottom: ".45rem" }}>
                    <div style={{ fontWeight: 800, fontSize: ".9rem" }}>Policy #25 Multi-Driver Execution</div>
                    <span className={`badge ${policy25Readiness.badgeClass}`}>{policy25Readiness.status}</span>
                  </div>
                  <div className="meta" style={{ marginBottom: ".55rem" }}>
                    <span>{POLICY25_MULTI_DRIVER_SOURCE}</span>
                    <span>Trigger: 3+ active drivers</span>
                    {policy25Readiness.additionalDriverPreRunReviewDue && <span>Additional-driver review due</span>}
                  </div>
                  <div style={{ fontSize: ".82rem", color: T.mu, marginBottom: ".55rem" }}>
                    This policy remains inactive at current capacity. Policy #8, Policy #16, and the parent pickup/delivery capability policies continue to govern active driver work.
                  </div>
                  <div style={{ fontSize: ".78rem", color: T.mu }}>
                    Pre-activation blockers: {policy25Readiness.blockers.join("; ")}
                  </div>
                </div>
                <div style={{ border: `1px solid ${T.border}`, borderRadius: 8, padding: ".75rem" }}>
                  <div className="card-head" style={{ marginBottom: ".45rem" }}>
                    <div style={{ fontWeight: 800, fontSize: ".9rem" }}>Policy #26 Courier Billing</div>
                    <span className={`badge ${policy26Readiness.badgeClass}`}>{policy26Readiness.status}</span>
                  </div>
                  <div className="meta" style={{ marginBottom: ".55rem" }}>
                    <span>{POLICY26_COURIER_BILLING_SOURCE}</span>
                    <span>Trigger: external couriers or per-run model</span>
                    <span>Placeholder only</span>
                  </div>
                  <div style={{ fontSize: ".82rem", color: T.mu, marginBottom: ".55rem" }}>
                    No external-courier payment rows, client billing variants, or courier payment cycles are created while this conditional policy is inactive.
                  </div>
                  <div style={{ fontSize: ".78rem", color: T.mu }}>
                    Activation blockers: {policy26Readiness.blockers.join("; ")}
                  </div>
                </div>
              </div>
            </div>
            <div className="card">
              <div className="card-title" style={{ marginBottom: ".7rem" }}>Edit APP-ADM-006 Driver Record</div>
              <div className="fr">
                <div className="f"><label>Driver</label>
                ...
                <div className="f"><label>Engagement Start Date *</label><input type="date" value={driverDraft.engagementStartDate || ""} onChange={e => setDriverDraft(p => ({ ...p, engagementStartDate: e.target.value }))} /></div>
              </div>
              <div className="fr">
              ...
              <div className="fr">
                ...
                <div className="f"><label>Licence Number</label><input value={driverDraft.licenceNumber || ""} onChange={e => setDriverDraft(p => ({ ...p, licenceNumber: e.target.value }))} placeholder="Licence evidence reference" /></div>
              </div>
              <div className="fr">
                <div className="f"><label>Licence Class / Vehicle Class</label><input value={driverDraft.licenceClass || ""} onChange={e => setDriverDraft(p => ({ ...p, licenceClass: e.target.value }))} placeholder="Australian licence class appropriate for van" /></div>
                <div className="f"><label>Admin Identifier</label><input value={driverDraft.policy13AdminIdentifier || ""} onChange={e => setDriverDraft(p => ({ ...p, policy13AdminIdentifier: e.target.value }))} placeholder="Admin who performed Policy #13 checks" /></div>
              </div>
              <div className="fr">
                <div className="f"><label>Original Licence Sighted</label><input type="date" value={driverDraft.licenceOriginalSightedDate || ""} onChange={e => setDriverDraft(p => ({ ...p, licenceOriginalSightedDate: e.target.value }))} /></div>
                <div className="f"><label>Licence Verified Date</label><input type="date" value={driverDraft.licenceVerifiedDate || ""} onChange={e => setDriverDraft(p => ({ ...p, licenceVerifiedDate: e.target.value }))} /></div>
              </div>
              <div className="fr">
                <div className="f"><label>Licence Outcome</label>
                  <select value={driverDraft.licenceVerificationOutcome || ""} onChange={e => setDriverDraft(p => ({ ...p, licenceVerificationOutcome: e.target.value }))}>
                    <option value="">Not verified</option>
                    <option value={POLICY13_LICENCE_OUTCOME_VERIFIED}>{POLICY13_LICENCE_OUTCOME_VERIFIED}</option>
                    <option value="Blocked - licence issue unresolved">Blocked - licence issue unresolved</option>
                  </select>
                </div>
                <div className="f"><label>Licence Copy Retained</label>
                  <label style={{ display: "flex", alignItems: "center", gap: ".5rem", fontSize: ".82rem", color: T.tx, textTransform: "none", letterSpacing: 0, fontWeight: 700 }}>
                    <input type="checkbox" checked={Boolean(driverDraft.licenceCopyRetained)} onChange={e => setDriverDraft(p => ({ ...p, licenceCopyRetained: e.target.checked }))} />
                    Copy retained on driver file
                  </label>
                </div>
              </div>
              <div className="fr">
                <div className="f"><label>Photo ID Verified Date</label><input type="date" value={driverDraft.photoIdVerifiedDate || ""} onChange={e => setDriverDraft(p => ({ ...p, photoIdVerifiedDate: e.target.value }))} /></div>
                <div className="f"><label>Photo ID Outcome</label>
                  <select value={driverDraft.photoIdVerificationOutcome || ""} onChange={e => setDriverDraft(p => ({ ...p, photoIdVerificationOutcome: e.target.value }))}>
                    <option value="">Not verified</option>
                    <option value={POLICY13_PHOTO_ID_OUTCOME_VERIFIED}>{POLICY13_PHOTO_ID_OUTCOME_VERIFIED}</option>
                    <option value="Blocked - photo ID unresolved">Blocked - photo ID unresolved</option>
                  </select>
                </div>
              </div>
              <div className="fr">
                <div className="f"><label>Criminal-History Review Date</label><input type="date" value={driverDraft.criminalHistoryReviewedDate || ""} onChange={e => setDriverDraft(p => ({ ...p, criminalHistoryReviewedDate: e.target.value }))} /></div>
                <div className="f"><label>Criminal-History Outcome</label>
                  <select value={driverDraft.criminalHistoryOutcome || ""} onChange={e => setDriverDraft(p => ({ ...p, criminalHistoryOutcome: e.target.value }))}>
                    <option value="">Not reviewed</option>
                    <option value={POLICY13_CRIMINAL_HISTORY_OUTCOME_CLEAR}>{POLICY13_CRIMINAL_HISTORY_OUTCOME_CLEAR}</option>
                    <option value="Blocked - disqualifying or unresolved history">Blocked - disqualifying or unresolved history</option>
                  </select>
                </div>
              </div>
              <div className="fr">
                <div className="f"><label>Right To Work Verified Date</label><input type="date" value={driverDraft.rightToWorkVerifiedDate || ""} onChange={e => setDriverDraft(p => ({ ...p, rightToWorkVerifiedDate: e.target.value }))} /></div>
                <div className="f"><label>Right To Work Outcome</label>
                  <select value={driverDraft.rightToWorkOutcome || ""} onChange={e => setDriverDraft(p => ({ ...p, rightToWorkOutcome: e.target.value }))}>
                    <option value="">Not verified</option>
                    <option value={POLICY13_RIGHT_TO_WORK_OUTCOME_VERIFIED}>{POLICY13_RIGHT_TO_WORK_OUTCOME_VERIFIED}</option>
                    <option value="Blocked - right to work unresolved">Blocked - right to work unresolved</option>
                  </select>
                </div>
              </div>
              <div className="fr">
                <div className="f"><label>Annual Licence Review Date</label><input type="date" value={driverDraft.licenceCurrencyReviewedDate || ""} onChange={e => setDriverDraft(p => ({ ...p, licenceCurrencyReviewedDate: e.target.value }))} /></div>
                <div className="f"><label>Known Licence Issue</label>
                  <label style={{ display: "flex", alignItems: "center", gap: ".5rem", fontSize: ".82rem", color: T.tx, textTransform: "none", letterSpacing: 0, fontWeight: 700 }}>
                    <input type="checkbox" checked={Boolean(driverDraft.knownLicenceIssue)} onChange={e => setDriverDraft(p => ({ ...p, knownLicenceIssue: e.target.checked }))} />
                    Remove from run schedule until resolved
                  </label>
                </div>
              </div>
              <div className="f"><label>Licence Issue Note</label><input value={driverDraft.licenceIssueNote || ""} onChange={e => setDriverDraft(p => ({ ...p, licenceIssueNote: e.target.value }))} placeholder="Required evidence if a licence issue is known" /></div>
              <div className="fr">
                <div className="f"><label>Classification</label><input value={driverDraft.classification || ""} onChange={e => setDriverDraft(p => ({ ...p, classification: e.target.value }))} placeholder="Leave blank until legal classification confirmed" /></div>
                <div className="f"><label>Classification Status</label>
                  <select value={driverDraft.classificationStatus || DRIVER_CLASSIFICATION_BLOCKER} onChange={e => setDriverDraft(p => ({ ...p, classificationStatus: e.target.value }))}>
                    <option value={DRIVER_CLASSIFICATION_BLOCKER}>Blocked pending legal classification</option>
                    <option value="Recorded from legal advice">Recorded from legal advice</option>
                  </select>
                </div>
              </div>
              <div className="fr">
                <div className="f"><label>Agreement Signed Date</label><input type="date" value={driverDraft.agreementSignedDate || ""} onChange={e => setDriverDraft(p => ({ ...p, agreementSignedDate: e.target.value }))} /></div>
                <div className="f"><label>Agreement Status</label>
                  <select value={driverDraft.agreementStatus || DRIVER_AGREEMENT_BLOCKER} onChange={e => setDriverDraft(p => ({ ...p, agreementStatus: e.target.value }))}>
                    <option value={DRIVER_AGREEMENT_BLOCKER}>Blocked pending Policy #12 legal review</option>
                    <option value="Signed">Signed</option>
                  </select>
                </div>
              </div>
              <div className="fr">
                <div className="f"><label>Engagement End Date</label><input type="date" value={driverDraft.engagementEndDate || ""} onChange={e => setDriverDraft(p => ({ ...p, engagementEndDate: e.target.value }))} /></div>
              ...
              <div className="f"><label>Onboarding Status</label><input value={driverDraft.onboardingStatus || DRIVER_ONBOARDING_BLOCKER} onChange={e => setDriverDraft(p => ({ ...p, onboardingStatus: e.target.value }))} /></div>
              <div className="f"><label>Notes</label><textarea value={driverDraft.notes || ""} onChange={e => setDriverDraft(p => ({ ...p, notes: e.target.value }))} placeholder="Evidence reference, review notes, or known blocker" /></div>
              <div className="f"><label>Driver Record Change Reason *</label><input value={driverChangeReason} onChange={e => setDriverChangeReason(e.target.value)} placeholder="Required for APP-PRV-004 audit trail" /></div>
              <button className="btn b-acc b-sm" onClick={saveDriverRecordDraft}>Save Driver Record</button>
            </div>
            <div className="stats">
            ...
            {driverRecordRows.map(({ driver: d, gaps, policy13, onboardingBlocked, retentionUntil }) => (
              <div className="card" key={d.id}>
                <div className="card-title" style={{ marginBottom: ".4rem" }}>{d.name}</div>
                <div className="meta" style={{ marginBottom: ".35rem" }}>
                  <span className={`badge ${driverRecordBadgeClass(d)}`}>{gaps.length ? "Action Required" : (onboardingBlocked ? "Legal Blocker" : "Ready")}</span>
                  <span className={`badge ${policy13.badgeClass}`}>Policy #13 {policy13.status}</span>
                  <span>Driver ID {d.id}</span>
                </div>
                <div className="meta"><span>✉️ {d.email}</span><span>📞 {d.phone}</span></div>
                <div className="meta" style={{ marginTop: ".35rem" }}>
                  <span>Engagement start {d.engagementStartDate ? fmtFullDate(d.engagementStartDate) : "Required now"}</span>
                  <span>Licence {d.licenceNumber || "Not recorded"}</span>
                  <span>Licence class {d.licenceClass || "Not recorded"}</span>
                  <span>Last reviewed {d.lastReviewed ? fmtFullDate(d.lastReviewed) : "Not recorded"}</span>
                </div>
                <div className="meta" style={{ marginTop: ".35rem" }}>
                  <span>Licence check {d.licenceVerifiedDate ? fmtFullDate(d.licenceVerifiedDate) : "Not verified"}</span>
                  <span>Photo ID {d.photoIdVerifiedDate ? fmtFullDate(d.photoIdVerifiedDate) : "Not verified"}</span>
                  <span>Criminal-history review {d.criminalHistoryReviewedDate ? fmtFullDate(d.criminalHistoryReviewedDate) : "Not reviewed"}</span>
                  <span>Right to work {d.rightToWorkVerifiedDate ? fmtFullDate(d.rightToWorkVerifiedDate) : "Not verified"}</span>
                  <span>Admin {d.policy13AdminIdentifier || "Not recorded"}</span>
                </div>
                <div className="meta" style={{ marginTop: ".35rem" }}>
                  <span>{d.classification || d.classificationStatus || DRIVER_CLASSIFICATION_BLOCKER}</span>
                  <span>{d.agreementSignedDate ? `Agreement signed ${fmtFullDate(d.agreementSignedDate)}` : (d.agreementStatus || DRIVER_AGREEMENT_BLOCKER)}</span>
                  <span>{d.onboardingStatus || DRIVER_ONBOARDING_BLOCKER}</span>
                </div>
                {policy13.gaps.length > 0 && <div style={{ fontSize: ".78rem", color: T.acc, marginTop: ".35rem" }}>Policy #13 blocks APP-ADM-002 assignment: {policy13.gaps.join("; ")}</div>}
                {gaps.length > 0 && <div style={{ fontSize: ".78rem", color: T.acc, marginTop: ".35rem" }}>APP-ADM-006 gaps: {gaps.join("; ")}</div>}
                {onboardingBlocked && <div style={{ fontSize: ".78rem", color: T.mu, marginTop: ".35rem" }}>Production onboarding remains blocked by SOP-JDD-01 until legal classification and Policy #12 Driver Agreement are confirmed.</div>}
                {retentionUntil ? (
                  <div style={{ fontSize: ".78rem", color: T.mu, marginTop: ".35rem" }}>Policy #5 retention: engagement end + 7 years, due {fmtFullDate(retentionUntil)}.</div>
                ) : (
                  <div style={{ fontSize: ".78rem", color: T.mu, marginTop: ".35rem" }}>Policy #5 retention: duration of engagement plus 7 years; retention clock starts when engagement end is recorded.</div>
                )}
                ...
                <div style={{ display: "flex", gap: ".6rem", flexWrap: "wrap", marginTop: ".7rem" }}>
                  <button className="btn b-ghost b-sm" onClick={() => editDriverRecord(d)}>Edit APP-ADM-006 Record</button>
                </div>
              </div>
            ))}
...
export default function App() {
  ...
  function recordDriverMasterDataChanges(previous, driver, reason, action) {
    const fields = [
      ...
      ["licenceOriginalSightedDate", "licence_original_sighted_date"],
      ["licenceCopyRetained", "licence_copy_retained"],
      ["licenceVerifiedDate", "licence_verified_date"],
      ["licenceVerificationOutcome", "licence_verification_outcome"],
      ["photoIdVerifiedDate", "photo_id_verified_date"],
      ["photoIdVerificationOutcome", "photo_id_verification_outcome"],
      ["criminalHistoryReviewedDate", "criminal_history_reviewed_date"],
      ["criminalHistoryOutcome", "criminal_history_outcome"],
      ["rightToWorkVerifiedDate", "right_to_work_verified_date"],
      ["rightToWorkOutcome", "right_to_work_outcome"],
      ["licenceCurrencyReviewedDate", "licence_currency_reviewed_date"],
      ["knownLicenceIssue", "known_licence_issue"],
      ["licenceIssueNote", "licence_issue_note"],
      ["policy13AdminIdentifier", "policy_13_admin_identifier"],
      ["classification", "classification"],
      ["classificationStatus", "classification_status"],
      ["engagementStartDate", "engagement_start_date"],
      ["engagementEndDate", "engagement_end_date"],
  ...
  function saveDriver(driver, reason = "Not recorded") {
    ...
    writeAudit(exists ? "APP-ADM-006 driver record updated" : "APP-ADM-006 driver record added", `${storedDriver.name || storedDriver.id}: ${reason}`, "admin");
  }
