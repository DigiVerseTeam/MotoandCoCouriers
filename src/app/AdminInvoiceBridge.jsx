...
export default function AdminInvoiceBridge() {
  ...
  async function createInvoice(group) {
    ...
    try {
      ...
      if (email) {
        ...
        setMessage(`Validated, created and emailed Books invoice ${result.invoiceNumber || result.invoiceId} to ${sent.toMailIds?.join(", ") || email}.`);
      } else {
        setMessage(`Validated and created Books invoice ${result.invoiceNumber || result.invoiceId}. Add a billing email next time to send it from the app.`);
      }
      await refresh();
    } catch (error) {
  ...
  return (
    <div className="ai-bridge">
      ...
      {open && (
        <section className="ai-panel" aria-label="Month-end account invoicing">
          <div className="ai-head">
            <div>
              <h2>Month End</h2>
              <p>Review delivered milk runs, validate they happened, then create and email account invoices.</p>
            </div>
          ...
          {invoiceGroups.length === 0 ? (
            <div className="ai-empty">No delivered billable jobs waiting for month-end invoicing.</div>
          ) : invoiceGroups.map(group => (
            <article className="ai-group" key={group.key}>
              <div className="ai-group-head">
                <div><h3>{group.accountName}</h3><p>{monthLabel(group.period)} - {group.deliveries.length} delivered job{group.deliveries.length === 1 ? "" : "s"}</p></div>
                <strong>${group.total.toFixed(2)}</strong>
              </div>
              <label className="ai-check">
                <input type="checkbox" checked={Boolean(validated[group.key])} onChange={event => setValidated(previous => ({ ...previous, [group.key]: event.target.checked }))} />
                <span>I have reviewed this account/month and validate that these milk run deliveries happened.</span>
              </label>
              ...
              <button className="ai-create" type="button" disabled={busyKey === group.key || !validated[group.key] || (emails[group.key] && !validEmail(emails[group.key]))} onClick={() => createInvoice(group)}>
                {busyKey === group.key ? "Creating..." : emails[group.key] ? "Validate, Create & Email Invoice" : "Validate & Create Invoice"}
              </button>
            </article>
          ))}
        </section>
